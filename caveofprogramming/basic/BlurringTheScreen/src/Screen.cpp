...
bool Screen::init()
{
	...
	// initialize buffer with zero values (here 0xFF = 255 = color white)
...
void Screen::boxBlur()
{
	// Swap the buffers, so pixel is in m_bufer2  and we are drawing to m_buffer1.
			...
			for (int row = -1; row <= 1; ++row) {
				for (int col = -1; col <= 1; ++col) {
					...
					if (currentX >=0 && currentX < SCREEN_WIDTH && currentY >= 0 && currentY < SCREEN_HEIGHT) {
						Uint32 color = m_buffer2[currentY*SCREEN_WIDTH+currentX]; //get color
